...
- [ ] 1.1 触发一次重试，对比 `backend/data/bonita.log` 实际内容与 Logs 页面 WebSocket 接收到的内容，确认是否丢失 Celery 写入的日志
- [ ] 1.2 检查 Celery worker 进程的 `LOGGING_LOCATION` 相对路径解析（uvicorn 与 celery 的工作目录是否一致），如不一致在 `config.py` 改为绝对路径或基于 `BASE_DIR` 拼接
- [ ] 1.3 验证 `logs.py:93` 的正则 `r"\[(.*?)\] (\w+) in ([\w\.]+): (.*)"` 是否能匹配 Celery task logger 输出（如 `celery.task`、`celery.worker`），不能匹配则放宽 module 部分（如 `([\w\.]+)`）
- [ ] 1.4 在 Records 页面和 Logs 页面之间加一个跳转链接作为临时缓解（待 scrape_log 抽屉上线后可移除）
...
- [ ] 4.4 手动测试：`GET /api/v1/records/all?success=false` 同时返回 False 与 NULL 记录，`count` 正确
...
- [ ] 5.4 测试 API：用 curl 或 OpenAPI UI 验证两个端点的鉴权与返回格式
...
- [ ] 7.7 端到端测试：触发一次重试，确认 `scrape_log` 表有完整记录，`log_text` 包含过程日志

## 8. 后端：scrape_log 过期清理任务

- [ ] 8.1 在 `backend/bonita/celery_tasks/tasks.py` 新增 `@shared_task(name='cleanup:scrape_logs')` 任务，删除 30 天前的 scrape_log（但保留每条 record 最新一条 success 日志）
- [ ] 8.2 在 Celery beat 配置中注册每日触发（参考现有 beat 调度配置位置）
- [ ] 8.3 测试：手工插入过期数据，触发任务验证清理结果
...
- [ ] 13.1 端到端测试场景 A：未刮削页签 → 重试选中 → 抽屉查看日志 → 日志完整显示刮削过程
- [ ] 13.2 端到端测试场景 B：任务开启 `skip_on_success` → `POST /tasks/run/{id}`（path 为空）全量重跑 → 确认 `success=True` 记录被跳过、日志可见
- [ ] 13.3 端到端测试场景 C：`force_refresh=True` 重试路径 → 确认即使 `skip_on_success=True` 也不跳过
- [ ] 13.4 端到端测试场景 D：scrape_log 保留策略 - 单 record 插入 21 条后自动保留 20 条最新；30 天前数据被 beat 任务清理
- [ ] 13.5 更新 `README.md` 或部署文档，说明新增的 `skip_on_success` 字段语义与 scrape_log 表用途
- [ ] 13.6 在 `openspec` 中运行 `openspec validate records-tabs-and-scrape-logs` 确认所有 spec 完整
