...
- [x] 9.1 调用 `GET /api/v1/records/all?success=true`，确认只返回成功记录，count 与过滤后总数一致
- [x] 9.2 调用 `GET /api/v1/records/all?success=false`，确认只返回失败记录
- [x] 9.3 调用 `GET /api/v1/records/all`（不传 success），确认返回所有记录（向后兼容）
- [x] 9.4 打开 Records 页面，在状态筛选中选择"成功"，确认表格只显示成功记录；切换回"全部"恢复
- [x] 9.5 点击列选择按钮，取消勾选若干列，确认对应列从表格中消失；重新勾选后恢复
- [x] 9.6 确认"名称"和"操作"列的 checkbox 为禁用状态，无法取消
- [x] 9.7 设置状态筛选、隐藏部分列、切换排序，刷新页面，确认设置被恢复
- [x] 9.8 在搜索框输入文字后刷新页面，确认搜索框为空（搜索词未持久化）但状态筛选/列可见性保持
- [x] 9.9 清除 localStorage 中的 `records-view-settings`，刷新页面，确认使用默认设置
